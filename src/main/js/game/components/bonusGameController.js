define(require => {
    ...
    function setupWheel() {
        ...
        wheelTimeLine.to(topWheel.scale, 1, {x: bonusWheelTopScale[1], y: bonusWheelTopScale[1]}, 0);
        wheelTimeLine.to(middleWheel.scale, 1, {x: bonusWheelMiddleScale[1], y: bonusWheelMiddleScale[1]}, 0);
        wheelTimeLine.to(bottomWheel.scale, 1, {x: bonusWheelBottomScale[1], y: bonusWheelBottomScale[1]}, 0);

        wheelTimeLine.to(bottomWheel, 1, {pixi: {brightness: 0.5}}, 0);
        wheelTimeLine.to(middleShadow, 1, {alpha: 0}, 0);

        wheelTimeLine.addPause(1);

        wheelTimeLine.to(topWheel.scale, 1, {x: bonusWheelTopScale[2], y: bonusWheelTopScale[2]}, 1);
        wheelTimeLine.to(middleWheel.scale, 1, {x: bonusWheelMiddleScale[2], y: bonusWheelMiddleScale[2]}, 1);

        wheelTimeLine.to(middleWheel, 1, {pixi: {brightness: 0.5}}, 1);
        wheelTimeLine.to(topShadow, 1, {alpha: 0}, 1);
    }
    ...
    function spinData() {
        msgBus.publish('game.wheel.spin', {target: target, endpoint: bonusArray[0], wheelIndex: wheelIndex});
        displayList.bonusSpinButton.enabled = false;
    }
